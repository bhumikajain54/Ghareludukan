  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        {
          id: "shop-approvals",
          label: "Shop Onboarding Queue",
          icon: Store,
          badge: metrics.pendingShopApprovals || 0,
        },
        {
          id: "delivery-approvals",
          label: "Delivery Partner KYC",
          icon: Bike,
          badge: metrics.pendingDeliveryApprovals || 0,
        },
        {
          id: "fraud-investigation",
          label: "Fraud & Trust Desk",
          icon: ShieldAlert,
          badge: metrics.fraudAlertsCount || 0,
        },
      ],
    },
    {
      ...
      items: [
        { id: "approved-shops", label: "Verified Merchants", icon: CheckCircle2 },
        { id: "orders", label: "Platform Orders", icon: ShoppingBag },
        { id: "settlements", label: "Payout Settlements", icon: CreditCard },
        { id: "audit-logs", label: "Immutable Audit Logs", icon: FileText },
        { id: "reports", label: "GMV & SLA Reports", icon: BarChart3 },
      ],
    },
    {
      group: "SYSTEM",
      items: [
        { id: "notifications", label: "Alerts & Events", icon: Bell, badge: unreadNotifs },
  ...
  const renderSidebar = (isExpanded) => (
    <div className="flex flex-col h-full bg-slate-900 text-slate-100 select-none overflow-hidden">
      ...
      <div className="flex items-center px-4 py-3.5 border-b border-slate-800 h-16 shrink-0 overflow-hidden">
        <div className="w-10 h-10 rounded-xl bg-cyan-600 flex items-center justify-center shadow-lg shadow-cyan-950/60 shrink-0">
        ...
        >
          ...
          <div className="text-[9px] font-mono text-cyan-400 uppercase tracking-widest flex items-center gap-1 font-bold truncate">
            <Sparkles size={9} /> Platform Admin
          </div>
        </div>
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {badge && (
                      <span
                        className={`ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-cyan-500 text-slate-950 min-w-[18px] text-center transition-all duration-200 ${
                    ...
                    {/* Dot Badge (Collapsed) */}
                    {badge && !isExpanded && (
                      <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-cyan-400 rounded-full border-2 border-slate-900" />
                    )}
                  </button>
                );
              })}
...
export function AdminBottomNav({ view, onNav, pendingCount = 0 }) {
  const items = [
    { id: "dashboard", label: "Home", Icon: Home },
    { id: "shop-approvals", label: "Shops", Icon: Store, badge: pendingCount },
    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-900 border-t border-slate-800 flex items-center justify-around px-2 py-2 safe-area-bottom">
      {items.map(({ id, label, Icon, badge }) => {
        ...
        return (
          ...
          >
            {badge > 0 && (
              <span className="absolute top-0 right-1 text-[9px] font-bold w-4 h-4 bg-cyan-500 text-slate-950 rounded-full flex items-center justify-center">
                {badge > 9 ? "9+" : badge}
              </span>
            )}
            <Icon size={20} />
